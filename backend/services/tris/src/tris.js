...
checkEnvVariables(['INTERNAL_API_KEY', 'PORT', 'NOTIFICATION_SERVICE_URL', 'USERS_SERVICE_URL', 'MAX_MARKS_PER_PLAYER',
	'READY_COOLDOWN_MS', 'MOVE_TIMEOUT_MS' , 'MATCHMAKING_IGNORE_BLOCKS', 'EARNED_WIN_POINTS', 'LOST_LOSS_POINTS', 'PLACEHOLDER_DELETED_USERNAMES' ]);
...
const	start = async () =>
{
	try
	{
        ...
        console.log(`[TRIS] Server is running on ${HOST}:${process.env.PORT}`)
		console.log(`[TRIS] Web socket is listening on ws://${HOST}:${process.env.PORT}/ws`)
	}
	catch (err)
	{
